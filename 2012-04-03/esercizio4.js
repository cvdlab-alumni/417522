...
var buildChair = function () {
	var gamba1 = SIMPLEX_GRID([ [0.05], [0.05], [0.4] ]);
	var gamba2 = SIMPLEX_GRID([ [-0.35, 0.05], [0.05], [0.4] ]);
	var gamba3 = SIMPLEX_GRID([ [0.05], [-0.35, 0.05], [0.7] ]);
	var gamba4 = SIMPLEX_GRID([ [-0.35, 0.05], [-0.35, 0.05], [0.7] ]);
	...
	var sedile = SIMPLEX_GRID([ [0.4], [0.4], [-0.3,0.05] ]);
	
	var schienale1 = SIMPLEX_GRID([ [-0.05, 0.3], [-0.35,0.05], [-0.6, 0.1] ]);
	var schienale2 = SIMPLEX_GRID([ [-0.05, 0.3], [-0.35,0.05], [-0.4, 0.1] ]);
	var schienale3 = SIMPLEX_GRID([ [-0.1, 0.05], [-0.35, 0.05], [-0.5,0.1] ]);
	var schienale4 = SIMPLEX_GRID([ [-0.25, 0.05], [-0.35,0.05], [-0.5, 0.1] ]);

	var schienale = STRUCT([schienale1, schienale2, schienale3, schienale4]);

	var cuscino = SIMPLEX_GRID([ [-0.05,0.3], [-0.05,0.3], [-0.35,0.02] ]);
	COLOR([236/255, 83/255, 0])(cuscino);

	var sedia = STRUCT( [gambe, schienale, sedile] );

	COLOR( [77/255, 48/255, 38/255] )(sedia);

	sedia = STRUCT( [gambe, schienale, sedile, cuscino] );

	sedia = S([0,1,2])([1.2,1.2,1.2])(sedia);
...
var placeChair = function (v,chair) {
	var x = v[0];
	var y = v[1];
	...
	chair = T([0])([x])(chair);
	chair = T([1])([y])(chair);
	chair = T([2])([thickness])(chair);

	return chair;
}

var chair = buildChair();;
chair = placeChair([26,13],chair);
